...
## Inspect Reddit ingestion failures
...
## Reddit ingestion via GitHub Actions

Reddit blocks anonymous `.json` requests from Supabase Edge Function egress, so
the scheduled Reddit fetch runs from GitHub Actions instead.
...
The workflow is `.github/workflows/fetch-reddit-threads.yml`. It runs hourly,
but the script only fetches at 6 AM through 8 PM Eastern time, every two hours.
Manual workflow dispatch runs immediately for testing.
